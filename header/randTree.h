int RandTime(BiNode *T,double fatherTime){
   ...
   // printf("access node %s\n",T->name);
    ...
    T->NodeTime=temp+r;
   // printf("node time is %.2lf\n",temp);
    RandTime(T->lchi,T->NodeTime);
    RandTime(T->rchi,T->NodeTime);
...
int randstore[128][100]={0};
int randlength[128]={0};
int randindex=0;
...
BiNode* DoRerrange(BiNode* T){
    ...
    if (T->lchi->NodeTime>T->rchi->NodeTime){
        ...
        gchi2=T->rchi->rchi;
        r=rand()%2;
        randstore[randindex][randlength[randindex]]=r;
        randlength[randindex]++;
        if (r==0)
