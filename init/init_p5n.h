//
#define DATADAYS 6
double datatime[DATADAYS+1]={0,303,713,1057,1414,1813,2149};
...
BiNode *init()
{
    ...
    char *name;
    int   L;
    char *filename="fasta/haplotypes_p5_p17.fasta";  //
	...
	while(ReadFASTA(ffp, &seq, &name, &L)){
        ...
        //
	    if (strcmp(S[n]->name,"days_303_")==0) {S[n]->NodeTime=datatime[1];datanum[1]++;}
	    else if (strcmp(S[n]->name,"days_713_")==0) {S[n]->NodeTime=datatime[2];datanum[2]++;}
	    else if (strcmp(S[n]->name,"days_1057")==0) {S[n]->NodeTime=datatime[3];datanum[3]++;}
	    else if (strcmp(S[n]->name,"days_1414")==0) {S[n]->NodeTime=datatime[4];datanum[4]++;}
        else if (strcmp(S[n]->name,"days_1813")==0) {S[n]->NodeTime=datatime[5];datanum[5]++;}
	    else if (strcmp(S[n]->name,"days_2149")==0) {S[n]->NodeTime=datatime[6];datanum[6]++;}
