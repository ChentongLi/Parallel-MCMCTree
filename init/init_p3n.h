//
#define DATADAYS 9
double datatime[DATADAYS+1]={0,146,501,797,1126,1476,1934,2006,2344,3079};
...
BiNode *init()
{
    ...
    char *name;
    int   L;
    char *filename="fasta/haplotypes_p3_p17.fasta";  //
	...
	while(ReadFASTA(ffp, &seq, &name, &L)){
        ...
        //
	    if (strcmp(S[n]->name,"days_146_")==0) {S[n]->NodeTime=datatime[1];datanum[1]++;}
	    else if (strcmp(S[n]->name,"days_501_")==0) {S[n]->NodeTime=datatime[2];datanum[2]++;}
	    else if (strcmp(S[n]->name,"days_797_")==0) {S[n]->NodeTime=datatime[3];datanum[3]++;}
	    else if (strcmp(S[n]->name,"days_1126")==0) {S[n]->NodeTime=datatime[4];datanum[4]++;}
	    else if (strcmp(S[n]->name,"days_1476")==0) {S[n]->NodeTime=datatime[5];datanum[5]++;}
	    else if (strcmp(S[n]->name,"days_1934")==0) {S[n]->NodeTime=datatime[6];datanum[6]++;}
	    else if (strcmp(S[n]->name,"days_2006")==0) {S[n]->NodeTime=datatime[7];datanum[7]++;}
	    else if (strcmp(S[n]->name,"days_2344")==0) {S[n]->NodeTime=datatime[8];datanum[8]++;}
